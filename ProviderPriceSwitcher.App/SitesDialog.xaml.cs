...
public sealed partial class SitesDialog : Window
{
    ...
    private async Task AddSiteAsync() { if (_busy) return; var dialog = _editorFactory.Create(null, Settings, this); if (dialog.ShowDialog() == true && dialog.Site is not null) await SaveSiteAsync(dialog.Site, null); }
