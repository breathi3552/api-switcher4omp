...
public sealed partial class SitesDialog : Window
{
    ...
    private void UpdateButtons() { var selected = Selected; _edit.IsEnabled = !_busy && selected is not null; _toggle.IsEnabled = !_busy && selected is not null; _delete.IsEnabled = !_busy && selected is not null; _toggle.Content = selected?.Enabled == true ? "禁用" : "启用"; }
    private async Task AddSiteAsync() { if (_busy) return; var dialog = _editorFactory.Create((SiteConfiguration?)null, Settings, this); if (dialog.ShowDialog() == true && dialog.Site is not null) await SaveSiteAsync(dialog.Site, null); }
