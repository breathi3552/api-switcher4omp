...
- Core 保存价格、快照、成本和推荐所需的环境无关领域模型与计算；Application 编排价格检查、设置管理、活动路由应用/恢复、独立的 OMP GPT 配置替换和 OMP 启动；Adapters 封装供应商协议；Infrastructure 提供文件、Windows 凭据、OMP 配置、sidecar 和进程等外部能力；App 负责 WPF 装配与呈现。
...
6. 活动路由持久化只保存非秘密 `ActiveProviderId`；恢复时供应商必须仍启用且 API key 仍存在并绑定当前分组，否则清空且不自动回退。OMP GPT 配置替换是独立的预览/确认文件迁移；OMP 启动只负责保存工作目录并创建新实例，已有 OMP 实例只提供信息，不阻止新实例启动。
...
- OMP GPT 配置替换和重复启动已实现：替换只扫描主 `config.yml` 的约定模型角色引用，按 `/` 后模型名的 `gpt` 不区分大小写前缀筛选，支持 `provider-price-switcher` 与 `openai-codex`；本地目标仅局部维护自有 `models.yml` Provider 定义，官方目标完全不读写 `models.yml`，配置写入后提示手动重启 OMP。每次启动请求均尝试创建新实例，不改变活动供应商。目标 loopback 端口保存后在下次完整应用启动时迁移，端口冲突不自动换端口或连接未知进程。
