...
设置保存 MUST 复用 `SettingsUseCase(ISettingsRepository).Save`。它以 `StringComparer.OrdinalIgnoreCase` 去重工作目录并保留首次拼写，优先保留等价的原默认目录，否则选择第一项或 null；不额外检查目录是否存在。新增 UI 流程不得直接写 JSON repository。`SitesDialog` 接收预构造的 `SiteManagementUseCase`、`IPricingSnapshotQuery` 与 editor factory；`SiteEditorViewModel` 接收预构造的 `PricingProbeUseCase`，两级对话框工厂均由 composition root 提供。
