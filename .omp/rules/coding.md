...
- 不可预期异常 MUST 保留原始异常作为 inner exception 并在应用边界统一记录/通知；不得 `catch (Exception)` 后静默忽略、伪装成功或自动切换 Provider。
...
- 日志 SHOULD 使用结构化参数和稳定事件语义；禁止用消息字符串承担错误分类。异常记录 MUST 保留异常对象，事件级别 MUST 与可恢复性相符。
...
- 审查标准：对每个新增日志事件逐字段检查来源和敏感性；对异常和网络响应检查是否可能被隐式序列化；确认日志路径可由隔离数据根测试验证。
