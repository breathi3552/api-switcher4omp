...
10. 人工验收边界：工具无法可靠证明 owner 的责任归属、status 是否与真实组织进度一致、priority 是否符合当前治理排序，以及路线分类是否符合团队意图时，必须在关闭前列出具体人工验收点：由指定 owner/维护者逐条确认登记记录的 owner、status、priority 和 last_verified；按每条 `evidence`/`DoD` 相对链接打开并确认其内容确实支撑该记录；审阅 Project Overview 的质量路线/产品路线是否没有把治理工作误写成产品承诺。每个点记录操作人、日期、期望结果、实际结果、风险和未验原因；任一点未验或不通过，保持 `open` 并记录阻塞，不得笼统写“请人工验证”。若上述组织性确认已有可审计工具证据，则明确记录“无需人工验收”及其证据来源。
11. 关闭条件与未通过处理：代码闭环必须同时满足不可变性、旧 API 零引用、刷新无持久化转发和 JSON 兼容；文档闭环必须同时满足字段完整、链接无死链且唯一、质量/产品路线分离和状态事实一致。任一代码或文档条件、人工验收点、隔离安全约束未满足，均不得关闭；保留具体失败输出、阻塞项和重跑入口，修复后从对应验证步骤重新执行。
