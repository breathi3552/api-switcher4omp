...
try
{
    ...
    Assert(defaults.OmpRootDirectory == expectedRoot && defaults.OmpConfigPath == Path.Combine(expectedRoot, "agent", "config.yml"), "OMP root defaults and derived config path");
    ...
    Assert(migrated.OmpRootDirectory == "C:\\Users\\test\\.omp" && migrated.OmpConfigPath == "C:\\Users\\test\\.omp\\agent\\config.yml", "legacy OMP config path migration");
    var pawsRoot = new SiteConfiguration
    {
        ProviderId = "paws-root", DisplayName = "保留名称", ConfigurationKey = "old-root-key",
        BaseUrl = new Uri("https://ai.furry.edu.gr/"), SiteType = "new-api", Enabled = false,
        Model = "Model-X", CurrentGroup = " group ", CurrentGroupRatio = 2.5m, GroupRatioSource = "手动",
        AuthenticationMode = "令牌", Currency = "CNY", CnyConversionRate = 7.2m
    };
    ...
    {
        ProviderId = "sevnx", DisplayName = "SevnX", ConfigurationKey = "old-sevnx-key",
        BaseUrl = new Uri("https://www.sevnx.one/login"), SiteType = "new-api", Enabled = true,
        Model = "gpt-5.6-sol", CurrentGroup = "default", CurrentGroupRatio = 1.5m, GroupRatioSource = "手动",
    ...
    {
        ProviderId = provider, ConfigurationKey = "k", Model = "gpt-5.6-sol", CurrentGroup = "g",
        BasePrices = new TokenPrices { InputPerMillion = value, CachedInputPerMillion = 0, OutputPerMillion = 0 },
        CurrentGroupRatio = 1,
        Prices = new TokenPrices { InputPerMillion = value, CachedInputPerMillion = 0, OutputPerMillion = 0 }, RefreshedAt = DateTimeOffset.UtcNow
    };
